...
/*int col_checker(int arr[N][N], char argv[N][N])
{
    ...
    while (x < N)
    {
        col1 = argv[0][x] - '0';
        col2 = argv[1][x] - '0';
        if (flag == 1 && coldown_counter(arr, col1, x) == 1 && coldown_counter(arr, col2, x) == 1)
            flag = 1;
        else:
            flag = 0;
        x++;
...
/*int row_checker(int arr[N][N], char argv[N][N])
{
    ...
    while (x < N)
    {
        row1 = argv[2][x] - '0';
        row2 = argv[3][x] - '0';
        if (flag == 1 && rowleft_counter(arr, row1, x) == 1 && rowright_counter(arr, row2, x) == 1)
            flag = 1;
        else:
            flag = 0;
        x++;
...
int main_checker(int arr[N][N], char argv[N][N])
{
    ...
    while (x < N)
    {
        if (col_flag && row_flag && rowleft_counter(arr, row1, x) && rowright_counter(arr, row2, x))
            if (coldown_counter(arr, col1, x) && colup_counter(arr, col2, x))
                return (1);
    }
    return (0);
}
...
{
    int x;
    if (colnum == N && rownum == N - 1 && main_checker(arr, argv) == 1)
        return (1);
    if (colnum == N)
    {
        rownum++;
